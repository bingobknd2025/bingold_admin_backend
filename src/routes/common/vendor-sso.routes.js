...
router.post("/sso/register",
  /*  #swagger.tags = ['Vendor SSO']
      ...
      #swagger.requestBody = { required:true, content: { "application/json": { schema: { type:'object', required:['fullName','shopName','shopSlug','email','phone','password','confirmPassword','countryId'], properties: { fullName:{type:'string'}, email:{type:'string'}, phone:{type:'string'}, password:{type:'string', description:'Strong: 8+ chars incl. upper/lower/number/special'}, confirmPassword:{type:'string', description:'Must match password'}, countryId:{type:'string', description:'Dialing code, e.g. 91'}, shopName:{type:'string'}, shopSlug:{type:'string'}, businessName:{type:'string'}, description:{type:'string'}, gstNumber:{type:'string'}, panNumber:{type:'string'}, supportEmail:{type:'string'}, supportPhone:{type:'string'} } }, example: { fullName:'Acme Owner', email:'owner@acme.com', phone:'9876543210', password:'Secret@123', confirmPassword:'Secret@123', countryId:'91', shopName:'Acme Store', shopSlug:'acme-store', businessName:'Acme Pvt Ltd', description:'We sell gadgets', gstNumber:'22AAAAA0000A1Z5', panNumber:'AAAAA0000A', supportEmail:'support@acme.com', supportPhone:'9876543211' } } } } */
  ctrl.register);
